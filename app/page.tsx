...
export default async function Home() {
  ...
  return (
    <div className="bg-background-primary text-foreground-primary">
      <section className="bg-gradient-to-br from-white via-accent-muted/30 to-white">
        <PageShell
        ...
        >
          <div className="grid gap-6 sm:grid-cols-3 animate-in fade-in slide-in-from-bottom-6 duration-700 delay-150">
            {highlights.map((item, index) => (
              <div 
                key={item.label} 
                className="card-hover rounded-xl border border-border bg-white p-6 shadow-card animate-fade-in-up"
                style={{ animationDelay: `${index * 100}ms` }}
              >
                <p className="text-4xl font-bold text-foreground-primary mb-2">{item.label}</p>
                <p className="text-xs font-medium text-foreground-secondary uppercase tracking-widest">{item.description}</p>
              </div>
            ))}
          </div>
        </PageShell>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {pillars.map((pillar, index) => (
            <Card 
              ...
              style={{ animationDelay: `${index * 100}ms` }}
            >
              <p className="text-base text-foreground-secondary leading-relaxed">{pillar.copy}</p>
            </Card>
          ))}
        </div>
      </PageShell>
      ...
      >
        <div className="grid gap-6 md:grid-cols-3">
          {steps.map((step, index) => (
            <div 
              key={step.title} 
              className="card-hover rounded-xl border border-border bg-white p-6 shadow-card"
            >
              <Pill className="bg-accent-muted text-primary">Phase {index + 1}</Pill>
              <h3 className="mt-5 section-title text-foreground-primary">{step.title}</h3>
              <p className="mt-3 text-base text-foreground-secondary leading-relaxed">{step.body}</p>
            </div>
          ))}
        </div>
      </PageShell>
      ...
      <PageShell subdued className="mt-0">
        <div className="rounded-2xl border border-border bg-gradient-to-br from-white to-accent-muted/30 p-12 text-center shadow-card">
          <h2 className="text-4xl font-bold text-foreground-primary mb-4">Hire or get hired with intent</h2>
