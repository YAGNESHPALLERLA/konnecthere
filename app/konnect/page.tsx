...
export default function KonnectPage() {
  ...
  // Message cache: store messages per conversation to prevent refetch/flicker
  ...
  // Debounce search input for user discovery
  useEffect(() => {
  ...
  // Fetch users for discover section
  useEffect(() => {
  ...
  // Fetch conversations once when authenticated
  useEffect(() => {
    if (status === "authenticated") {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        console.error("[KONNECT] Failed to fetch users:", { status: res.status, error: errorData })
      ...
      console.error("[KONNECT] Error fetching users:", { message: error?.message, stack: error?.stack })
  ...
  const fetchConversations = async () => {
    try {
      ...
      const data = await res.json()
      // Only include conversations that have a lastMessage (at least one message exchanged)
      const convs: Conversation[] = (data.conversations || []).filter(
  ...
  // Fetch messages for a conversation (only if not cached or on explicit refresh)
  const fetchMessages = useCallback(
    async (conversationId: string, showLoading = false) => {
      ...
      try {
        ...
        const data = await res.json()
        const messagesArray: Message[] = Array.isArray(data.messages)
          ? data.messages
        ...
        // Refresh conversations silently to update lastMessage / unread counts
        void fetchConversations()
      } catch (error: any) {
  ...
  // Handle selecting a conversation from the chat list
  ...
  // Poll messages for selected conversation (silent updates, no flicker)
  useEffect(() => {
  ...
  }, [selectedConversation, status, fetchMessages])

  // Filter conversations for chat list
  useEffect(() => {
  ...
  // Start a conversation from the discover list
  ...
  const isLoadingFirstTime =
    selectedConversation && !initialLoad[selectedConversation] && loadingMessages
